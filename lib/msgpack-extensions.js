...
codec.addExtPacker(0x02, packAs.Uuid, (data) => {
  console.log('[EXT-PACKER UUID] Упаковка UUID:', data.value);
  var result = uuidParse(data.value);
  console.log('[EXT-PACKER UUID] Результирующий буфер:', result);
  return result;
});

codec.addExtUnpacker(0x02, (buffer) => {
  console.log('[EXT-UNPACKER UUID] Распаковка UUID из буфера:', buffer);
  var result = uuidStringify(buffer);
  console.log('[EXT-UNPACKER UUID] Результат:', result);
  return result;
});
...
codec.addExtPacker(0x04, Date, (date) => {
  console.log('[EXT-PACKER DATETIME] Упаковка даты:', date);
  ...
  console.log('[EXT-PACKER DATETIME] Результирующий буфер:', buffer);
  return buffer;
});

codec.addExtUnpacker(0x04, (buffer) => {
  console.log('[EXT-UNPACKER DATETIME] Распаковка даты из буфера:', buffer);
  ...
  console.log('[EXT-UNPACKER DATETIME] Результат:', time);
  return time;
});
