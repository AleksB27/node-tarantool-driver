...
Commands.prototype.upsert = function(spaceId, ops, tuple){
	...
	return new Promise(function (resolve, reject) {
		if (Array.isArray(ops)){
			...
			var reqId = _this._getRequestId();
			const { customCodec } = require('./customCodec'); // убедитесь, что путь правильный
			var bufTuple = require('msgpack-lite').encode(tuple, { codec: customCodec });
			var bufOps = msgpackEncode(ops, {codec});
...
Commands.prototype.insert = function(spaceId, tuple){
    console.log('[INSERT] Входной кортеж:', tuple);
    var reqId = this._getRequestId();
...
Commands.prototype._replaceInsert = function(cmd, reqId, spaceId, tuple){
    var _this = this;
    console.log('[REPLACE] Входной кортеж:', tuple);
    return new Promise(function (resolve, reject) {
        if (Array.isArray(tuple)){
            if (typeof(spaceId) == 'string') {
                return _this._getMetadata(spaceId, 0)
                    .then(function(info){
                        console.log('[REPLACE] _getMetadata info:', info);
                        console.log('[REPLACE] Tuple до рекурсивного вызова _replaceInsert:', tuple);
                        return _this._replaceInsert(cmd, reqId, info[0], tuple);
                    })
                    .then(resolve)
                    .catch(reject);
            }
            // Логирование непосредственно перед кодированием:
            console.log('[REPLACE] Tuple перед кодированием:', tuple);
            console.log('[REPLACE] tuple[0]:', tuple[0]);
            console.log('[REPLACE] tuple[1]:', tuple[1]); // здесь должен быть ваш UUID объект
            console.log('[REPLACE] tuple[2]:', tuple[2]);
            
            const { customCodec } = require('./customCodec'); // убедитесь, что путь правильный
	    var bufTuple = require('msgpack-lite').encode(tuple, { codec: customCodec });
            console.log('[REPLACE] Результирующий буфер после msgpackEncode:', bufTuple);
            ...
            console.log('[REPLACE] Финальный буфер для отправки:', buffer);
            _this.sendCommand([
                cmd, 
                reqId, 
                { resolve: resolve, reject: reject }
