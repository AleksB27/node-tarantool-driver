...
Commands.prototype.update = function(spaceId, indexId, key, ops){
	...
	if (Number.isInteger(key))
		key = [key];
	    // console.log("DEBUG: update - raw key:", key);
    if (Array.isArray(key)) {
        key.forEach(function(k, i) {
            //console.log("DEBUG: key[" + i + "] type:", typeof k, "is Buffer:", Buffer.isBuffer(k));
            if (Buffer.isBuffer(k)) {
              //  console.log("DEBUG: key[" + i + "] hex:", k.toString('hex'));
            }
        });
    }
	return new Promise(function (resolve, reject) {
...
Commands.prototype._replaceInsert = function(cmd, reqId, spaceId, tuple){
    ...
    return new Promise(function (resolve, reject) {
        if (Array.isArray(tuple)){
            ...
            const { customCodec } = require('./customCodec'); 
	    var bufTuple = require('msgpack-lite').encode(tuple, { codec: customCodec });
