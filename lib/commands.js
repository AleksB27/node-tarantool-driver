  ...
  return new Promise(function (resolve, reject) {
    if (Array.isArray(tuple)){
      if (typeof(spaceId)=='string') {
        return _this._getMetadata(spaceId, 0)
          .then(function(info){
            console.log('[COMMANDS] _getMetadata info:', info);
            console.log('[COMMANDS] Tuple до рекурсивного вызова _replaceInsert:', tuple);
      ...
      console.log('[COMMANDS] Перед кодированием, tuple:', tuple);
      console.log('[COMMANDS] tuple[0]:', tuple[0], '; tuple[1]:', tuple[1], '; tuple[2]:', tuple[2]);
