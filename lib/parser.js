...
exports._processResponse = function(buffer, offset){
  console.log('[PARSER] Входящий буфер:', buffer.toString('hex')); // лог буфера
  decoder.buffer = buffer;
  decoder.offset = offset || 0;
  var headers = decoder.fetch();
  console.log('[PARSER] Заголовки:', headers);	
  ...
  console.log('[PARSER] Данные:', data);	
  ...
  if (success) {
	var result = _returnBool(task[0], data);
	console.log('[PARSER] Успешный результат:', result);
	dfd.resolve(result);
  }
	else {
    ...
    console.error('[PARSER] Ошибка:', errorDecription);
    if ([
